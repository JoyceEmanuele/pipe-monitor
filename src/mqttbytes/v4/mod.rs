...
pub fn read_2(stream: &mut [u8], max_size: usize) -> Result<(usize, Packet), Error> {
    let fixed_header = check(stream.iter(), max_size)?;

    // Test with a stream with exactly the size to check border panics
    let frame_length = fixed_header.frame_length();
    let packet = Bytes::from(stream[0..fixed_header.frame_length()].to_owned());
    let packet_type = fixed_header.packet_type()?;

    if fixed_header.remaining_len == 0 {
        // no payload packets
        return match packet_type {
            PacketType::PingReq => Ok((frame_length, Packet::PingReq)),
            PacketType::PingResp => Ok((frame_length, Packet::PingResp)),
            PacketType::Disconnect => Ok((frame_length, Packet::Disconnect)),
            _ => Err(Error::PayloadRequired),
        };
    }
