...
async fn tunnel_server_listener(listen_socket: &str, globs: Arc<GlobalVars>) -> Result<(), String> {
	...
	loop {
		...
		let (tls_config, tls_type) = (ConnectionAcceptor::Rustls, "Rustls");
...
async fn socket_forwarder2<T1: AsyncRead+AsyncWrite, T2: AsyncRead+AsyncWrite>(device_stream: T1, broker_stream: T2, dev: Arc<ClientInfo>, globs: Arc<GlobalVars>, i_broker: usize) -> Result<(Result<(), String>, Result<(), String>), String> {
	...
	let client_to_server_fut = async {
		...
		let ident = dev.ident.lock().await;
		println!("[{} {} {} {}/{}] Finalizando transferência {}: {:?}", &(chrono::offset::Local::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, false)[..19]), dev.addr, direction, ident.client_id, ident.dev_id, direction, r1);
		if ident.client_id.is_empty() {
			globs.stats_counters.b_stats[i_broker].tls_err.fetch_add(1, Ordering::Relaxed);
		}
		match wo.shutdown().await {
			Ok(()) => { dev.ended_server_w.store(true, Ordering::Relaxed); },
			Err(err) => { println!("[{} {} {} {}/{}] Não foi possível dar shutdown no socket de escrita para o broker: {}", &(chrono::offset::Local::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, false)[..19]), dev.addr, direction, ident.client_id, ident.dev_id, err); },
		};
		r1
	};
	...
	let server_to_client_fut = async {
		...
		let ident = dev.ident.lock().await;
		println!("[{} {} {} {}/{}] Finalizando transferência {}: {:?}", &(chrono::offset::Local::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, false)[..19]), dev.addr, direction, ident.client_id, ident.dev_id, direction, r2);
		match wi.shutdown().await {
			Ok(()) => { dev.ended_client_w.store(true, Ordering::Relaxed); },
			Err(err) => { println!("[{} {} {} {}/{}] Não foi possível dar shutdown no socket de escrita para o device: {}", &(chrono::offset::Local::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, false)[..19]), dev.addr, direction, ident.client_id, ident.dev_id, err); },
		};
		r2
	};
		...
		let packet = match mqtt_reader.read_packet(r, max_packet_size, mqtt_timeout).await {
			Ok(packet) => {
				...
				match &packet {
					...
					mqttbytes::v4::Packet::PubRec(_p) => {},
					mqttbytes::v4::Packet::PubRel(_p) => {},
					mqttbytes::v4::Packet::PubComp(_p) => {},
				...
				// O trecho abaixo foi adicionado em 2024-04 para tentar entender o problema de desconexões de alguns dispositivos
				if ["DAM305230212","DAM308220431","DAM308221162","DAM308221160","DAM310228500","DAM399232854","DRI008220080"].contains(&&dev_id[..]) {
					println!("[{} {} {} {}/{}] {:?}", &(chrono::offset::Local::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, false)[..19]), dev.addr, direction, client_id, dev_id, packet);
				}
				...
				match &packet {
					...
					mqttbytes::v4::Packet::Publish(p) => {
						if direction == "dv<-br" {
							...
							if p.dup {
								globs.stats_counters.b_stats[i_broker].pub_dup.fetch_add(1, Ordering::Relaxed);
							}
						} else {
